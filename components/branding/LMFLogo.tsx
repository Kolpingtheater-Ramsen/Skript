interface LMFLogoProps {
  className?: string;
  size?: 'sm' | 'md' | 'lg';
  color?: string;
}

export function LMFLogo({ className = '', size = 'md', color = '#d90429' }: LMFLogoProps) {
  const sizes = {
    sm: { width: 80, height: 48 },
    md: { width: 125, height: 75 },
    lg: { width: 251, height: 150 },
  };
  const { width, height } = sizes[size];

  return (
    <svg className={className} width={width} height={height} viewBox="0 0 2510 1500" fill={color} xmlns="http://www.w3.org/2000/svg">
